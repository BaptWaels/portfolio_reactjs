  ...
  getSkillChip (skill, index) {
    ...
    return (
      <Chip className='skill-chip' key={index}>
        <ChipContact style={{ background: `url(${imgUrl}) 0 0 / cover` }} />
        {skill.name}
      </Chip>
    )
  }
    ...
    return (
      <Dialog className='projectDialog' open={data.toggle} onCancel={this.handleCloseDialog.bind(this)}>
        ...
        <div className='content'>
          ...
          <div className='skills'>{project.skills.map(this.getSkillChip.bind(this))}</div>
          ...
          <Grid className='description'>
            <Cell col={8} className='description-section'>
              ...
              <div className='desc'>{project.description.section.content[intl.locale]}</div>
            </Cell>

            <Cell col={4} className='client'>
              <h2>{project.description.client.title}</h2>
              <div className='desc'>{project.description.client.content[intl.locale]}</div>
            </Cell>
          </Grid>
        </div>
      </Dialog>
